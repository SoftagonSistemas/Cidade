...
<template>
  <v-container>
    <v-row>
      <v-col cols="12">
        <v-data-table
          :items="items" :search="search" item-value="id" show-select class="elevation-1" pagination
          :headers="headers"
        >
          <template #top>
            <v-toolbar flat>
              ...
              <v-menu>
                ...
                <v-list>
                  <v-list-item
                    v-for="criteria in ['Priority', 'Due Date', 'Created', 'Updated']" :key="criteria"
                    @click="() => { }"
                  >
          ...
          <template #item.reply="{ item }">
            <v-btn v-if="item.reply === true" color="success" icon="mdi-reply" size="xs" variant="text" />
          </template>
          <template #item.ticket="{ item }">
            <v-chip color="primary" size="small" to="/admin/tickets/ticket-details">
              {{ item.ticket }}
            </v-chip>
          </template>
          ...
          <template #item.actions="{ item }">
            <v-btn size="small" icon="mdi-pencil" variant="text" color="primary" @click="editItem(item)" />
            <v-btn size="small" icon="mdi-delete" variant="text" color="error" @click="deleteItem(item)" />
          </template>
        </v-data-table>
      </v-col>
    </v-row>
