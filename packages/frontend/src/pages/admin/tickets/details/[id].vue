...
async function loadTicketDetails() {
  try {
    loading.value = true
    ticket.value = await ticketService.getTicketDetails(ticketId)
    timelineItems.value = ticket.value.threads?.map((thread: any) => ({
      message: thread.message,
      color: 'primary',
      author: thread.user.name,
      datecreated: new Date(thread.createdAt).toLocaleString(),
      edit: true,
      createTicket: true,
      createTask: true,
      status: 'Updated Call',
...
async function saveStatus() {
  try {
    await ticketService.updateTicketStatus(ticketId, dialogStatus.value, statusDescription.value)
    ...
    <template v-if="ticket">
      ...
      <v-row class="mb-2">
        <v-col cols="auto">
          ...
          <v-chip
            size="small"
            :color="ticket.status.name === 'Aberto' ? 'success' : 'primary'"
          >
            {{ ticket.status.name }}
          </v-chip>
        </v-col>
        ...
        <v-col cols="auto">
          ...
          <v-chip
            size="small"
            :color="ticket.priority.name === 'Urgente' ? 'error' : 'warning'"
          >
  ...
  <v-dialog v-model="statusDialog" max-width="500px">
    <v-card>
      ...
      <v-card-text>
        <v-form>
          <!-- Status Dropdown -->
          <v-select v-model="dialogStatus" :items="['Complete', 'Close', 'Active', 'Pending']" label="New Status" outlined required />

          <!-- Description -->
          <v-textarea v-model="statusDescription" label="Description" rows="4" outlined required />
        </v-form>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
        ...
        <v-btn color="primary" @click="saveStatus">
          Save
        </v-btn>
      </v-card-actions>
    </v-card>
